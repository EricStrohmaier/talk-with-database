  ...
  for (const user of users) {
    ...
    try {
      if (storyItems.length === 0) {
        ...
        console.log(`${targetUser.username}'s story has ${storyItems.length} items`);
        storyItems.forEach(async (storyItem) => {
          await delay(1000);
          if (storyItem.media_type === 1 && storyItem.image_versions2) { // Check if it's an image
            const imageUrl = storyItem.image_versions2.candidates[0].url; // Highest quality image URL
            console.log(`${targetUser.username}'s image URL: ${imageUrl}`);
            await saveStoryItemToSupabase(targetUser.username, imageUrl, 'image');

          } else if (storyItem.media_type === 2) {
            const videoUrl = storyItem.video_versions[0].url;
            console.log(`${targetUser.username}'s video URL: ${videoUrl}`);
            await saveStoryItemToSupabase(targetUser.username, videoUrl, 'video');
      ...
      console.error(`Error fetching stories for ${targetUser.username}:`, error);
      // Optionally, introduce a longer delay on error or implement additional error handling
    }
  }
}
...
export async function saveStoryItemToSupabase(username: string, mediaUrl: string, mediaType: string) {
  const { data, error } = await supabase
    .from('insta_bucket')
    .insert([
      { insta_name: username, media_url: mediaUrl, media_type: mediaType }
    ]);
